...
Computation of the intersection polygon consists of three parts. In the first part (Algorithm \ref{algo:colect_23_ip_triangle}), we compute intersection points
on the boundary of $S_3$ using the line-tetrahedron intersection algorithm for every of its sides. There are 0,1, or 2 intersection points
...
through all the sides and appending the points into $I$ provides us with incomplete intersection polygon but in the correct order.
\notePE{Comment on process point (Algorithm \ref{algo:process_point}), face relation indicator, I would move description of face table here, since I looked at algorithm1 here, from it skipped to process point and I got lost..}

\notePE{If IP lies on an edge(s) of $S_4$, we create a mark so we exclude the edges from further computation in Algorithm \ref{algo:collect_23_ip_edges}.}

\notePE{In process point, we set for all the face of the vertex, if we coming through the vertex in or out the $S_4$. I understand that the condition on line 10 means, that we do not overwrite info if there is another IP on that face..}

\notePE{I corrected the algorithm references. Please check, it is ok now..}
...
\begin{algorithm}
  ...
  \DontPrintSemicolon
  \KwIn{input data}
  \KwOut{List of IPs sorted output data}  
  $I =\emptyset$\tcp*{Intersection polygon.}
  $F(:,:)=-1$\tcp*{Unset entries of the face connection table.}
  \For{side $s$ of $S_3$}{ 
    \nllabel{line:1_3}
    $L$ = intersection($s, S_4$)  \tcp*{List of max.\ two points.}
    process point($L[0]$, 0)\; 
    process point($L[-1]$, 1)
    \notePE{Isn't it clearer to write the two process point lines into cycle over L?}
  }
  \lIf{ $I[0] == I[-1]$ }{remove $I[-1]$}
  \notePE{I am not sure if I understand the notation '($[-1]$)', can be explained?
The last line then checks if first and last IPs, if they are not duplicates..??}
...
\begin{algorithm}
  \SetKw{And}{And}
  \DontPrintSemicolon
  \KwIn{intersection point $p$, face relation indicator $i$}
  \KwOut{update of $I$ and $F$}
  \lIf{ $p \ne I[-1]$ }{append $p$ to $I$}
  \notePE{I am missing the check whether IP is vertex of $S_3$ as referred in text.
  So what if it is a vertex?}\;
  set $j$ to index of $p$ in $I$\;
  \lIf{ $p$ is on face $f$}{ F[i,f]=j}
  \uElseIf{ $p$ is on edge $e$}{
    compute sign of intersection($e$, $S_3$)\;
    sort faces $f_0$, $f_1$ coincident with $e$\;
    F[i, $f_i$]=j
  }
  \ElseIf{ $p$ is on vertex $v$ of $S_4$}{
      \For{face $f$ coincident with $v$}{
         \lIf{ $F[i,f]==-1$}{F[i,f]=j}
      }
  }
  mark all edges of $S_4$ the $p$ lies on

  \caption{process point( $p$, $i$ )}
  \label{algo:process_point}
\end{algorithm}
...
\begin{algorithm}
  ...
  \For{unmarked edge $e$ of $S_4$}{
    ...
    \Else($p$ on the vertex $v$ of $S_4$){       
      ...
      mark all edges of $S_4$ coincident with $v$
    }
  }
...
\begin{algorithm}
  ...
  \KwOut{polygon in $J$ in correct order \notePE{J should be K ?}}  
  $P=(1,\dots, n-1,0)$\tcp{array of sucessors}
  \For{ $f$ is face of $S_4$}{
    \lIf{$F[1,f] \ne F[1,f]$}{$P[ F[1,f] ] = F[0, f]$} 
    \notePE{condition should be $F[0,f] \ne F[1,f]$ ? I don't understand: successor indices should be switched?
    This way there would be same two indices..}
  }
  $i=0$
