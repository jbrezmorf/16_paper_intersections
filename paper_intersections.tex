...
We first compute all component-bulk mesh intersections that is (1d-3d and 2d-3d) and then the 1d-2d and 2d-2d intersections using the 3d mesh to get the intersection pair candidates.
Element intersections for the particular component and bulk mesh 
...
If the number of components $k$ is small and if the components are contained inside the bulk mesh, the total time of initialization may still be linear $O(kn)$ \notePE{I believe you ment $O(kN)$ here}. However,
...
The advancing front method requires the neigbouring information for the elements within the components as well as within the bulk mesh.
...
two polygons. Since every side of IP is on the boundary of either component or bulk element, we can distinguish the bulk and component edges. Correspondingly, we save new candidate pairs into a~\emph{bulk queue} or 
a~\emph{component queue} to be subsequently processed later.
The scheme of the advancing front algorithm is displayed by the scheme in \fig{fig:prolongation},
where the queues are represented by the yellow boxes.
%
\begin{figure}[!htb]
%   \vspace{0pt}
  \centering    
...
On the input, we consider a candidate pair provided by the initialization (the top-left corner of the scheme).
If a nonempty intersection is found, we look for new candidate pairs among the neighboring elements
(see the white block 'Cycle over ICs').
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{IC on the boundary of both $c$ and $b$} \\
            This situation includes both previously described cases. Therefore, we obtain new candidate pairs
            $(c', b)$, $c'\in C'$ and $(c, b')$, $b'\in B'$ and push them into the component queue and the bulk queue, respectively.
\end{enumerate}
...
At the end, we see that the candidate pairs are of two types: $(c, b')$ and $(c', b)$,
from which the first one goes into the bulk queue, trying to fully cover current component element 
$c$ before we proceed to the next one. We are able recognize the case when the component element 
does not lie inside the bulk elements with its full volume -- we end up in situation \ref{enum:prolong2},
but there are no more bulk neighboring elements to prolong the intersection to. Then we mark this component
element as \emph{open}, so it can be later found by initialization algorithm and possibly
covered by other bulk elements but as different component.
