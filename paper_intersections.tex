...
algorithm are compared on the meshes raising from a hydrogeological applications.
\end{abstract}
...
paths which allow much faster transport that can not be fully captured by eqivalent continuum.
...
In this paper we present a new approach to the mesh intersectionn problem based on the \plucker coordinates,
...
We shall demonstrate this on an example of a relative position of a line and triangle sides in \fig{fig:plucker_products}.
The permuted inner products have the same sign in cases a and c, which will be later used as an indicator
that an intersection inside the triangle exists.
If any $\pi_p\odot \pi_{s_i}$ is zero, as in case b, it means that the lines $p$ and $s_i$ are intersecting each other
(note that $s_i$ are considered as infinite lines at the moment).
Any other situation not depicted in \fig{fig:plucker_products} is charaterized by different signs of permuted inner products.
...
\notePE{I think adding figure with node and side numbering would make it easier to read.}
\begin{lemma}
...
Combining these two expressions and substituing for $\vc V_0-\vc V_2=\vc s_1$, we get for side $\vc s_1$
\[
...
\begin{enumerate}
    ...
    \item If any $w_i$ is less than $-\epsilon$ (see definition bellow), there is no intersection, return empty IP. 
          Compare \fig{fig:plucker_products} and \eqref{eq:bary_centric} for better insight.
...
of problems with possible unconsistent results of two different, but logically related predicates. Adaptive-precision evaluation of 
...
\begin{figure}[!htb]
    ...
    \caption{Possible cases processed in the function \emph{vertex faces}.
             Only the main features refered in text are denoted: tetrahedorn vertex $v$, edge $e$, face $f$ (stripes).}
...
\begin{itemize}
 ...
 The other two (non-degenerate) edges may have either the opposite sign (the plane is cutting $S_3$, see case b in \fig{fig:vertex_faces}) 
 or the same sign (the plane is touching $S_3$ at the edge $e$, see case a).
 ...
 see case c in \fig{fig:vertex_faces}. 
 ...
 see case d in \fig{fig:vertex_faces}.
 ...
 We add IC to $J$ and return any pair of faces. See case e in \fig{fig:vertex_faces}.
\end{itemize}
