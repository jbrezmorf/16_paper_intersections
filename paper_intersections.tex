...
is called the \emph{topological position} of the IC on $K$. Moreover, as every IC is result of some permuted
inner product of the \plucker coordinates (see Section \label{sec:1d-2d}), we store the sign of the product as well.
...
\begin{figure}[!htb]
    \centering    
    \subfloat[$\pi_p\odot \pi_i < 0 \;\forall i$]{
        \includegraphics[width=0.3\textwidth]{\figpath plucker_product_a.pdf}
    }
    \subfloat[$\pi_p\odot \pi_i = 0 \, \exists i$ \newline special case]{
...
\end{equation}
and a triangle $S_2$ given by vertices $(\vc V_0, \vc V_1, \vc V_2)$ 
...
\]
The point $\vc X$ is inside of $S_2$ if and only if $w_i>0$ for all $i=0,1,2$.
\end{proof}
...
\begin{enumerate}
    ...
    \item \label{item:coplanar} If all $w_i$ are less than $\epsilon$, the line is coplanar with the triangle, both objects are 
        ...
        Every pair $p$, $s_i$ is checked for an intersection on $S_2$ boundary either inside $s_i$ or at a vertex $V_i$, setting the topological info $\tau_T$ to 
...
If we obtain an IC during the calculation that may possibly be part a degenerated IP but
we can not decide it at the moment we put that IC into the list $J$. 
...
%   \vspace{0pt}
  \centering    
    \includegraphics[width=\textwidth]{\figpath prolongation_scheme_ip_jb_fix_italic.pdf}
