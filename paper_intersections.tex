...
\noteJB{References already in introduction, possible references to some computer graphics book for AABB and BIH}
Having the algorithms for element-element intersections we can proceed to the mesh intersection algorithm. 
We first compute all component-bulk mesh intersections that is (1d-3d and 2d-3d) and then the 1d-2d and 2d-2d intersections use the 3d mesh
to get the intersection pair candidates.  Element intersections for the particular component and bulk mesh 
are obtained in two phases: firstly, we look for the first two elements intersecting each other (initialization); 
...
We choose an arbitrary element of the component mesh and check it intersection with all elements of the bulk mesh.
The process is accelerated by computing the axis aligned bounding box (AABB) for every element and use intersection of the bounding boxes
...
For the small number of components $k$ and if they are contained inside the bulk mesh the total time of initialization may be still linear $O(kn)$. However,
...
\paragraph{Advancing Front Method}
The advancing front algorithm requires and employ the neigbouring information for the elements within the component as well as within the bulk mesh.
It can be viewed as the breadth first search algorithm for a graph which vertices are intersection polygons and edges are sides shared by
two polygons. Side every side of IP is on the boundary of either component or the bulk element we can distinguish bulk and component edges and correspondingly 
the \emph{bulk queue} and the \emph{component queue}.
...
The scheme of the advancing front algorithm is displayed by the scheme in \fig{fig:prolongation}. 
The main idea is to compute intersections for a component element with all possible bulk elements, and then move to a~next neighboring component element. 
So the bulk queue is emptied before the component queue (see yellow boxes).

On the input we consider a candidate pair provided be the initialization (top-left corner).
...
\begin{itemize}
    \item \textbf{IC on the boundary of $C$  and inside $B$} \\
            We find all the sides coincident with the \nface of IC on $C$ component element in which the IP lies (IP can be at node and connect more sides).
    ...
    \item \textbf{IC on the boundary of both $C$ and $B$} \\
    ...
    \item \textbf{IC inside the component element and on the boundary of the bulk element)} \\
