...
inner product of some \plucker coordinates (see Section \label{sec:1d-2d}), we store the sign of the product as well.
...
inside of the bulk mesh the FS+AF variant is the fastest and with the smallest growth as expected. On the other hand the BIH variant is constantly slower by 20\% than the BIH+AF variant. That is roughly related to the average fraction of the non-intersecting 3d element in the bounding box of a 2d element.
The slightly larger growth of the BIH algorithm is masked by the common superlinear growth with power about $1.1$ which we do not completely understand.
\noteJB{???}
...
Each of these meshes contains \notePE{what is the 9 number??} 28 fractures in 9 2d components and a single 1d component, the tunnel.
