...
\begin{algorithm}
  ...
  \SetKw{continue}{continue}
  \KwIn{input data \notePE{$S_2$ and $S_3$}}
  \KwOut{List \notePE{$I$} of ICs on sides of \notePE{$S_2$, what output data??} sorted output data}  
...
\begin{algorithm}  
  ...
  \DontPrintSemicolon
  \KwIn{\notePE{this should go to previous Alg.} $I$ with ICs on $S_2$ boundary, partially filled $F$}
...
in Algorithm \ref{algo:colect_23_ip_triangle}. Then we pass through once again skipping the edges with none or degenerate\notePE{not in algorithm} $L[e]$.
...
\begin{algorithm}
  ...
  \KwOut{\notePE{I don't like the word conincident here - vertex lies on the edge}$(x_1, x_2)$, $x_1, x_2 \in M_3$, coincident with $v$ and intersected by the plane of $S_2$}
