...
\begin{itemize}
    \item \textbf{IC on the boundary of $c$  and inside $b$} \\
            ...
            elements $C'$ neighbouring with $c$ over any side$s\in S$. And finally, we push all pairs $(c',b)$, $c'\in C'$ 
    ...
    \item \textbf{IC on the boundary of both $c$ and $b$} \\
            ...
            Then we get all neighbours $B'$ through all faces in $F$ as in the previous case.
            We push pairs $(c, b')$, $b'\in B'$ into the bulk queue and pairs $(c', b')$, $c'\in C'$, $b'\in B'$ into the component queue.
    \item \textbf{IC inside of $c$ anf on the boundary of $b$)} \\
            We proceed as in previous case, and push pairs $(c, b')$, $b'\in B'$ into the bulk queue.
\end{itemize}
\noteJB{Can we just use two (not excluded) cases one for IB on boundary of $c$ second for CI on boundary of $b$ ?}

We also see that the candidate pairs are of three types: $(c, b')$, $(c', b)$, $(c',b')$
from which only the first one goes into the bulk queue, trying to cover current component element $c$ before we proceed to the next one.

When the pair is taken from either of the queues we check if the pair was computed and possibly skip it. 
