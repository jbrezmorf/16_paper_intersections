...
\begin{algorithm}
  ...
  \nllabel{line:ip_first_vtx_duplicate}
  \lIf{ $I[0] == I[-1]$ }{remove $I[-1]$}
  
  $l=0$
  \For{$p \in I$}{
    set $j$ to index of $p$ in $I$\;
    \If{ $p$ is on boundary of $S_4$}{
      $x$ is vertex,edge, or face on which $p$ lies\; 
      \For{ $i=0,\dots, s[j]$}{
        $F[l, x]=j$\;
        $l=1-l$ \tcp*{switch from input to output and vice versa}
      }  
    }
  }  
\end{algorithm}
...
\begin{algorithm}  
  ...
  \For{unmarked edge $e$ of $S_4$}{
    ...
    \uIf{ $L[e]$ is on boundary of $S_3$ }{
         \uIf{ $L[e]$ is inside of $e$}{ 
             get $i$ such that $F[i,e] = k$, $k\ne -1$
         }
         \Else($p$ on the vertex $v$ of $S_4$){ 
             get $i$ such that $F[i,v] = k$, $k\ne -1$          
         }         
         $F[1-i, f_{1-i}] = k$\;
    }
    \Else{
         append $L[e]$ to $I$\;
         set $j$ to index of $L[e]$ in $I$\;
         F[0, $f_0$]=j\;
         F[1, $f_1$]=j\;
         mark all edges coincident with $L[e]$
    }   
  }  
\end{algorithm}
...
\begin{algorithm}
  ...
  \KwOut{faces $S_4$ intersected by the plane of $S_3$}
