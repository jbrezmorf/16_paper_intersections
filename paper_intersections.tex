...
Let us consider a line $p$ with parametric equation $\vc X = \vc A + t\vc u$, $t\in\R$ and a triangle $T$ given by vertices $(\vc V_0, \vc V_1, \vc V_2)$ 
...
\noteJB{Has to be completed yet, however we consider that we obtain 0,1, or 2 distinguish ICs for all cases including various touch cases. Anyway every IC results from 
some non-degenerate 1d-2d intersection. We also assume that the ICs are in order according to the orientation of the side which is
...
There might be two intersection corners maximally, which are finally checked that they belong o abscissas.
If the line has its boundary point inside the tetrahedron, the intersection line must be cut
and coordinates of intersection corner are interpolated.
...
A new candidate pair of elements is found during prologantion, based on the topological information of the intersection corner. There are 3 possible cases (applies both for 1D-3D and 2D-3D):
\begin{itemize}
    \item \textbf{IC lies on component element side and inside bulk element} \\
            We find all the sides of component element in which the IC lies (IC can be at node and connect more sides).
    ...
    \item \textbf{IC lies on component element side and on the surface of bulk element} \\
            We find all the faces of bulk element in which the IC lies (1 face, or 2 faces (IC on an edge), 
            or 3 faces (IC at a node)). We find the corresponding neighboring bulk elements over the faces and
            ...
            If the candidates pair has been investigated already, we skip it.
    \item \textbf{IC lies inside component element (therefore must be on the surface of bulk element)} \\
