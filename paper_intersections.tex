...
The grid intersection algorithms are crucial for several techniques that tries to overcome some limitations of the classical finite element method.
...
We assume that the meshes $\mathcal T_i$ are connected, that is the graph having elements for vertices and their nighbouring for edges is connected.
We also assume that no submesh intersect itself. Further we assume that only $\mathcal{T}_1$ have $d_1=3$ and $d_i < 3$ for $i>1$. 
...
and compute that intersection. Every (possibly degenerated) intersection polygon (IP) is determined by its corners called intersection corners (IC).
...
for the line-tetrahedron intersections. These are used as the building block for the triangle-triangle and the triangle-tetrahedron cases.
