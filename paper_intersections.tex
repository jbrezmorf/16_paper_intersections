...
Computation of IP consists of three stages: calculation of side-tetrahedron ICs (Algorithm \ref{algo:colect_23_ip_triangle}), 
calculation of edge-triangle ICs (Algorithm \ref{algo:collect_23_ip_edges}), oredering of ICs (Algorithm \ref{algo:order_23}).
The intersection corners appended to the list $I$ as they are computed however their order on the polygon boundary is 
...
and every IC $p$ is followed by the side that lies on $F_p[p]\in M_2\cup M_3$ (see \noteJB{possible figure}). The vertices of the polygon are numbered in the same 
way as is the order and orientation of the sides of $S_2$, that is counterclockwise around the interiror with normal pointing to us.
...
\begin{minipage}{\textwidth}
  ...
  \begin{minipage}[b]{0.5\textwidth}
    ...
    \captionof{figure}{Effectivity of our algorithms using \plucker coordinates in comparison to \notePE{***}.
                         One can see 1D-3D case on the left, 2D-3D on the right.}
...
using \plucker coordinates, red values \notePE{***}. The gained speed up factor is approximately 5 in 1D-3D case and 2 in 1D-3D. Further we provide estimated count of floating point operations for different approaches in the Table \ref{tab:fundamental_flops}.
...
\begin{center}
    ...
    \begin{tabular}{>{\centering\arraybackslash}p{20ex}
                    ...
                    >{\centering\arraybackslash}p{20ex}}
        \colorbox{NavyBlue}{\textcolor{white}{BIHsearch}} & \colorbox{orange}{BBsearch} & \colorbox{Goldenrod}{BIHonly} \\ \hline
