...
These algorithms are combined with the advancing front method which allows us to reuse \plucker coordinates and their products between neighboring
...
This representation is independent on choice of $A_p$ since $\vc{u}_p\times (\vc A_p + t \vc u) = \vc{u}_p\times \vc A_p$.
...
line intersect the triangle inside. If any $\pi_p\odot \pi_{s_i}$ is zero, as in the case (b), 
...
\begin{figure}[!htb]
    ...
    \caption{Different relative positions of the line $p$ and a triangle with sides $s_i$, $i=0,1,2$. 
            ...
            the line $p$ is coplanar with some side in the case (b).
    }
...
The algorithm depends on the parameter $\epsilon$ used as a common tolerance parameter for detection of ICs with special positions. First, it is used 
...
through the all intersection algorithms. Currently, we use just fixed value $\epsilon=10^{-9}$. It works on all realistic meshes we deal with, but we are aware 
of problems with possible inconsistent results of two different, but logically related predicates. Adaptive-precision evaluation of 
...
given indirectly by the \emph{successor tables} $F_c[:]$ and $F_f[:]$. Every side of the polygon that lies on \nface $x\in M_2\cup M_3$ is followed by an 
IC given by $F_c[x]$ and every IC $p$ is followed by the side of the intersection polygon that lies on the \nface $F_f[p]\in M_2\cup M_3$ (see \fig{fig:tracing}).
For every IC $u$, we shall also find two $n$-faces $x$, $y$ incident with the two sides of the resulting IP that are neighbouring with the IC $u$. 
The order of $n$-faces $x$, $y$ have to match the orientation of the IP which is the same as the orientation of $S_2$ triangle, 
...
with duplicity of ICs in vertices, we further mark by the backlink $F_c[y]=u$ the $n$-faces that success some IC but still do not poses its successors.
If $y$ already have the backlink we swap $x$ and $y$. The result is the {\emph set links} operation formalized in Algorithm \ref{algo:set_links}.
\begin{algorithm}
...
\label{sec:sides}
\algo{algo:colect_23_ip_triangle} computes all ICs on the boundary of $S_2$.  It passes through every side $s$ of the triangle $S_2$ 
and computes the line-tetrahedron intersection $L$. In the regular case ($\abs{L}=2$), we process each IC in $L$ (loop on line \ref{line:one side loop}).

The IC $p$ is added to the list $I$, vertices of $S_2$ added twice are merged at final stage. 
Then (line \ref{line:m4_object}) we identify the \nface $x\in M_3$ 
the point $p$ lies on and set the tables $F_c$, $F_f$. For the IC at the vertex of $S_2$ we set $x$ to that vertex regardless of its position on $S_3$.
The backward temporary link on the line \ref{line:back_link43} is used later in \algo{algo:collect_23_ip_edges} to allow proper continuation 
...
If the side is just touching $S_3$, we get $L$ with a single IC. These ICs will be rediscovered again in \algo{algo:collect_23_ip_edges} 
...
\begin{algorithm}
  ...
  \For{side $s$ of $S_2$}{ 
    ...
    \For{$p$ in $L$}{ \nllabel{line:one side loop}
        $p$ lies on $n$-faces $x\in M_2$ and $y\in M_3$\;
        \If{$\abs{L} = 1$}{
...
\begin{algorithm}  
  ...
  \For{edge $e$ of $S_3$ with regular $L[e]$}{
    ...
    \uIf{ $p$ is on the boundary of $S_2$ }{
         ...
         $q=F_c[x]$\tcp*{$q$ is already computed $p$}
         \lIf{$F_f[q]=x$}{$F_f[q]=f_1$} \nllabel{line:successor}
         \lElse{$F_c[f_0]=q$}  \nllabel{line:predecessor}
         $F_c[x]=-1$\tcp*{remove the backlink}
    }
    \Else{
         append $p$ to $I$\; 
         $F_c[f_0]=p,\ F_f[p]=f_1$\tcp*{overwrite the backlink} \nllabel{line:insideS2}
    }   
  }  
\end{algorithm}
...
\algo{algo:collect_23_ip_edges} uses the line-triangle intersection algorithm for the edges of $S_3$ (line \ref{line:12edges}).
...
in \algo{algo:colect_23_ip_triangle}. Then we pass through once again skipping the edges with none or degenerate $L[e]$.
...
\begin{itemize}
 ...
 at the vertex $v$, the polygon degenerates into a point and thus no connection information is necessary.
 We add IC to $J$ and return any pair of faces, see \fig{fig:vertex_faces}, (e) .
\end{itemize}
...
get its successor \nface $x = F_f(p)$ and its successor IC $q=F_q(x)$. Unless $x$ is vertex of $S_2$, we put $q$ into $K$ and repeat the process until 
