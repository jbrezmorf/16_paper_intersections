...
intersections use the bulk mesh to get the intersection pair candidates, the Section \ref{sec:compnents}.
...
\label{front}
The advancing front algorithm requires and employ the neigbouring information for the elements within the component mesh $\mathcal T_c$ 
...
intersection polygons and graph edges are sides shared by two polygons. Side every side of IP is on the boundary of either 
component or the bulk element, we can distinguish bulk and component edges and correspondingly the \emph{component queue} $Q_c$ and the \emph{bulk queue} $Q_b$.
...
%   \vspace{0pt}
  \centering    
    ...
    %\input{\figpath prolongation_scheme_ip.pdf_tex}
...
First, we mark all component elements $c\in\mathcal T_c$ unvisited. The for every unvisited element $c\in \mathcal T_c$, we mark it visited
then find intersection candidate pair $(c,b)$, $b\in\mathcal T_b$ 
and initialize the front tracking for every unvisited .
